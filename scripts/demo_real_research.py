"""Run a real deep research task with Tavily Search API and save the report.

Usage:
    python scripts/demo_real_research.py
    python scripts/demo_real_research.py --question "对比 PyTorch 和 TensorFlow 的生态"
    python scripts/demo_real_research.py --preset 1 --report-type detailed_report

This script uses source_mode="real" (not mock), so Tavily API Key must be
configured in .env. The generated report is saved to docs/examples/ for
reference in README.

Demonstrates: create task → plan (via Skill) → execute with real tools →
generate report → copy to docs/examples/
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Traceable Research Agent — real Tavily research demo"
    )
    parser.add_argument(
        "--question", "-q",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Report type (default: detailed_report)",
    )
    parser.add_argument(
        "--skill", "-s",
        type=str,
        default="deep_web_research",
        help="Skill name to use for planning (default: deep_web_research)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="Directory to copy the final report to (default: docs/examples/)",
    )
    args = parser.parse_args()

    question = args.question or PRESET_QUESTIONS[args.preset - 1]
    output_dir = Path(args.output_dir) if args.output_dir else ROOT / "docs" / "examples"

    # ── Pre-flight checks ────────────────────────────────────────────────
    if not settings.tavily_api_key:
        _print_stage("error", "TAVILY_API_KEY is not set in .env. "
                     "Real research requires a Tavily API key.")
        return 1

    _print_header("Traceable Research Agent — Real Tavily Research Demo")
    print(f"\n  Research Question: {question}")
    print(f"  Skill: {args.skill}")
    print(f"  Report Type: {args.report_type}")
    print(f"  Source Mode: real")
    ...
    # ── [2/5] Create task + plan (via Skill) ────────────────────────────
    _print_stage("plan", "[2/5] Creating task and generating plan via Skill...")
    with SessionLocal() as db:
        ...
        if run_after and run_after.report_path:
            ...
            _print_stage("error", "  No report_path in AgentRun.")
    ...
    # ── Copy report to docs/examples/ ───────────────────────────────────
    if report_text:
        output_dir.mkdir(parents=True, exist_ok=True)
        # Use preset-based filename
        slug_map = {
            1: "agent-orchestration-comparison",
            2: "llm-security-papers-2024",
            3: "vector-database-comparison",
        }
        slug = slug_map.get(args.preset, "custom-research")
        example_path = output_dir / f"{slug}.md"
        example_path.write_text(report_text, encoding="utf-8")
        _print_stage("done", f"  Report copied to: {example_path}")
