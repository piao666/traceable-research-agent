...
def main() -> None:
    ...
    assert_true("自由研究" in source and "学术文献综述" in source, "automatic research task prompts missing")
    ...
    assert_true('"skill_name": "auto"' in source, "frontend should delegate Skill routing to the Planner")
    assert_true('"execution_mode_override": None' in source, "frontend should delegate execution routing to the Planner")
    assert_true('st.markdown("**执行模式**")' not in source, "execution engine should not be a sidebar user choice")
    assert_true("_load_dynamic_skills" not in source, "internal Skill IDs should not be mixed into task prompts")
    assert_true("MEMORY_KIND_CN" in source and "MEMORY_STATUS_CN" in source, "memory metadata localization missing")
    assert_true("_memory_content_cn" in source, "historical memory content localization missing")
    assert_true('st.markdown("**任务场景**")' in source, "task scene label missing")
    assert_true("[data-testid=\"stSidebar\"] hr" in source, "compact sidebar divider styling missing")
