...
工作台展示计划接口的类型化只读字段 `execution_insights`、`execution_budget`：
许可名单、逐工具禁用／冷却／输入拦截、父子共享调用与 Token 额度、可选人民币
费用估值、停止原因，以及可跳转实际 Trace 的候选来源队列。页面读取不调用外部
服务、不为旧 Run 新建预算、不重写历史；接口失败不会显示成零来源或无限额度。
“可继续选择”不代表服务连通，也不保证到期自动重试。

审批页明确 GitHub 可选、真实研究禁止转 mock、显式空名单禁止执行，
并区分 Planned 固定步骤与 ReAct 动态恢复。证据／报告页将来源摘录单独标识，
展示 Snapshot 与 Trace 身份；候选 URL、可解析引用、来源摘录均不等于事实核查
通过。不提供浏览器端改密钥、预算或强制重试某个工具的按钮。

固定材料集成覆盖 GitHub 401 → 非 GitHub URL → 抓取 → 实际保存带引用报告 →
类型化页面接口；组件测试覆盖刷新、预算限制、取消和“报告 → 证据 → Trace”跳转。
...
当前完整离线 pytest 共收集 601 项：599 项通过、2 项按条件跳过、0 项失败，
外部网络尝试为零。前端最新基线仍为 R9 的 110 项测试、类型检查、Lint、构建通过，
隔离路由／固定数据检查 59 项通过；本轮后端／工具修复未重跑前端和浏览器验收。
