...
本项目还提供一个本地 HTTP JSON-RPC MCP Bridge，避免每个外部服务都必须自己提供 HTTP MCP：
...
Bridge 地址默认是 `http://127.0.0.1:9001/mcp`。`fake` 模式不需要外部 key，适合离线演示；`real` 模式会读取：
...
第一版 real mode 支持 Firecrawl 和 Exa；Context7 默认提供 fake adapter 与 HTTP adapter 预留，等确认真实 HTTP API/MCP 形态后再启用 `CONTEXT7_BASE_URL`。
